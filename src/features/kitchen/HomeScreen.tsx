        ...
        <main className="mobile-page-stack">
          <section className="panel mobile-page-section ds-page-header ds-subpage-header">
            ...
            <div className="filter-panel-stack">
              <article className="filter-option-card ds-card">
                <span className="section-kicker">难度</span>
                <strong>当前：{selectedDifficultyLabel}</strong>
                <div className="filter-option-grid" role="group" aria-label="按难度筛选">
                  {difficultyOptions.map((option) => (
                    <button
                      key={option}
                      type="button"
                      className={`filter-dropdown-option ${props.difficulty === option ? 'filter-dropdown-option-active' : ''}`}
                      onClick={() => props.onDifficultyChange(option)}
                    >
                      {option}
                    </button>
                  ))}
              ...
              <article className="filter-option-card ds-card">
                <span className="section-kicker">总时长</span>
                <strong>当前：{selectedTimeLabel}</strong>
                <div className="filter-option-grid" role="group" aria-label="按总时长筛选">
                  {timeOptions.map((option) => (
                    <button
                      key={option}
                      type="button"
                      className={`filter-dropdown-option ${props.timeLimit === option ? 'filter-dropdown-option-active' : ''}`}
                      onClick={() => props.onTimeLimitChange(option)}
                    >
                      {option === allTimeOption ? '全部' : `${option} 分钟内`}
                    </button>
                  ))}
                </div>
              </article>
              ...
              <div className="filter-panel-actions">
                <button
                  ...
                  type="button"
                  onClick={() => {
                    props.onDifficultyChange(allDifficultyOption)
                    props.onTimeLimitChange(allTimeOption)
                  }}
                >
                  清空筛选
                </button>
                <button className="primary-button" type="button" onClick={() => setHomeSubpage(null)}>
                  查看结果
                </button>
              </div>
            </div>
